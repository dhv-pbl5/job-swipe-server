...
/**
 * Converter which converts a list of object to a JSON Array string and vice
 * versa
...
public abstract class DbJsonArrayConverter<T> implements AttributeConverter<List<T>, String> {
     ...
     *         {"{\"key\": \"_key\", \"value\": \"_value\"}","{\"key\": \"_key\",
     *         \"value\": \"_value\"}"}
     */
    @Override
    public String convertToDatabaseColumn(List<T> attribute) {
        if (CommonUtils.isEmptyOrNullList(attribute))
            return null;
    ...
    public List<T> convertToEntityAttribute(String dbData) {
        var clazz = getClazz();
        if (clazz == null || CommonUtils.isEmptyOrNullString(dbData))
            return null;
        ...
        if (CommonUtils.isEmptyOrNullString(str.toString()))
            return null;
